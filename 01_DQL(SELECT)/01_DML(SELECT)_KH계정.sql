...
/*
    < 컬럼값을 통한 산술연산 >
    SELECT절 컬럼명 작성 부분에 산술연산 기술 가능 (이때, 산술연산된 결과가 조회됨)
...
-- 단, 값이 지저분한 이유는 DATE형식 년/월/일/시/분/초 단위로 시간정보까지도 관리하고있기 때문
--> 깔끔한 결과 => 함수적용하면 확인 가능! 추후에 배움

----------------------------------------------------------------------------------------------------
/*
    < 컬럼명에 별칭 지정하기 >
    산술연산을 하게되면 컬럼명 지저분해짐 이때 컬럼명으로 별칭 부여해서 깔끔하게 보여줌
    
    [표현법]
    컬럼명 별칭 / 컬럼명 AS 별칭 / 컬럼명 "별칭" / ** 컬럼명 AS "별칭"
    
    AS 붙이든 안붙이든 부여하고자 하는 별칭에 띄어쓰기 혹은 특수문자가 포함될 경우 반드시 쌍따옴표로 묶여야함
...
-- 유의사항 : DISTINCT 는 SELECT절에서 딱 한번만 기술 가능
/* 구문오류
...
/*
    < WHERE 절 >
    조회하고자 하는 테이블 특정 조건에 만족하는 데이터만 조회하고자 할 때 사용
...
-- EMPLOYEE 테이블에서 부서코드가 'D1'인 사원들 조회 (이때, 사원명, 급여, 부서코드 컬럼만 조회)
...
-- EMPLOYEE 테이블에서 재직중(ENT_YN 컬럼값이 'N')인 사원들의 사번, 이름, 입사일 조회
...
--WHERE 연봉 >= 50000000; 오류! (WHERE절에서는 SELECT 절에 작성된 별칭 사용 불가!)
...
-- EMPLOYEE 테이블에서 부서코드가 'D9'이면서 급여가 500만원 이상인 사원들의 사번, 사원명, 급여, 부서코드 조회
SELECT EMP_NO, EMP_NAME, SALARY, DEPT_CODE
FROM EMPLOYEE
...
FROM EMPLOYEE
--WHERE 3500000 <= SALARY <= 6000000; 오류 발생! 자바와 마찬가지임
--WHERE 3500000 <= SALARY AND SALARY <= 6000000;
WHERE SALARY >= 3500000 AND SALARY <= 6000000; -- 일반적으로는 이 순서로! (관례적)
...
-- 이름이 '하'로 끝나는 사원들의 사원명, 주민번호, 전화번호 조회
SELECT EMP_NAME, EMP_NO, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하';

-- 이름에 '하'가 포함된 사원들의 사원명, 주민번호, 전화번호 조회
SELECT EMP_NAME, EMP_NO, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%';
...
-- 전화번호 3번째 자리가 '1'인 사원들의 사번, 사원명, 전번, 이메일 조회
-- 와일드카드 : _(1글자), %(0글자 이상)
...
--> 데이터값으로 취급하고자 하는 값 앞에 나만의 와일드 카드를 제시하고 나만의 와일드 카드를 ESCAPE OPTION 으로 등록해야됨!
...
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE /*NOT*/ EMAIL NOT LIKE '___$_%' ESCAPE '$';
...
-- 3. EMPLOYEE테이블에서 이름에 '하'가 포함되어 있고 급여가 240만원 이상인 사원들의 사원명과 급여 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%' AND SALARY >= 2400000;

-- 4. DEPARTMENT테이블에서 해외 영업부인 부서들의 코드, 부서명 조회
SELECT DEPT_ID, DEPT_TITLE
FROM DEPARTMENT
...
WHERE NOT BONUS IS NULL; -- NOT은 컬럼명 또는 IS 뒤에서 사용 가능
...
-- 부서 배치를 아직 받지는 않았지만(DEPT_CODE 값이 NULL인), 보너스는 받는 사원(BONUS 값이 NULL이 아닌)들의 이름, 보너스, 부서코드 조회
...
SELECT *
FROM EMPLOYEE
WHERE (JOB_CODE = 'J7' OR JOB_CODE = 'J2') AND SALARY >= 2000000; -- AND가 OR보다 우선순위가 높아서 원하는 결과X
...
-- 2. 연봉(보너스미포함)이 3천만원 이상이고 보너스를 받지 않는 사원들의 사번, 사원명, 급여, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE SALARY*12 >= 30000000 AND BONUS IS NULL;

-- 3. 입사일이 '95/01/01' 이상이고 부서배치를 받은 사원들 사번, 사원명, 입사일, 부서코드 조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE, DEPT_CODE
FROM EMPLOYEE
WHERE HIRE_DATE >= '95/01/01' AND DEPT_CODE IS NOT NULL;

-- 4. 급여가 200만원 이상 500만원 이하이고 입사일이 '01/01/01'이상이고 보너스는 받지 않는 사원들의 사번, 사원명, 급여, 입사일, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, HIRE_DATE, BONUS
FROM EMPLOYEE
WHERE (SALARY >= 2000000 AND SALARY <= 5000000) AND HIRE_DATE >= '01/01/01' AND BONUS IS NULL;


-- 5. 보너스를 포함한 연봉이 NULL이 아니고 이름이 '하'가 포함되어있는 사원들의 사번, 사원명, 급여, 보너스포함연봉 조회
...
FROM EMPLOYEE -- 1 => 테이블명이 존재하지 않으면 여기서 오류 발생
...
/*
    ...
    FROM 조회하고자 하는 테이블명
    WHERE 조건식
    ORDER BY 정렬하고자 하는 컬럼|별칭|컬럼순번 [ASC|DESC] [NULLS FIRST | NULLS LAST]
    
    - ASC : 오름차순 정렬 (생략시 기본값)
    - DESC : 내림차순 정렬
    
    - NULLS FIRST : 정렬하고자하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치 (생략시 DESC일때의 기본값)
    - NULLS LAST : 정렬하고자하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치 (생략시 ASC일때의 기본값)
    
*/

SELECT *
FROM EMPLOYEE
--ORDER BY BONUS; 생략시 ASC 적용
--ORDER BY BONUS ASC; -- 오름차순 정렬일 때 기본적으로 NULLS LAST 구나!
--ORDER BY BONUS /*ASC*/ NULLS FIRST;
--ORDER BY BONUS DESC NULLS LAST;  -- 내림차순 정렬일 때 기본적으로 NULLS FIRST 구나!
ORDER BY BONUS DESC, SALARY ASC; -- 정렬기준 여러개 제시 가능(첫번재 기준의 컬럼값이 동일할 경우 두번째의 기준 컬럼가지고 정렬)

-- 전체사원의 사원명, 연봉 조회(연봉별로 내림차순 정렬 조회)
SELECT EMP_NAME, SALARY*12 AS "연봉"-- 2
FROM EMPLOYEE -- 1
--ORDER BY SALARY*12 DESC;
--ORDER BY 연봉 DESC; -- 3 => ORDER BY 절이 마지막 실행순서이기에 별칭 사용 가능
ORDER BY 2 DESC; -- 컬럼순번이 2번인 컬럼 내림차순 조회 ** (컬럼개수보다 큰 숫자 입력시 오류 발생)

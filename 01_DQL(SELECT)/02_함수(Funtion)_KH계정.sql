...
SELECT TRUNC(123.456) FROM DUAL; -- 위치지정 안하면 FLOOR와 동일
SELECT TRUNC(123.456, 1) FROM DUAL; -- 소수점 아래 첫째자리까지 표현하고싶다
...
-- * NEXT_DAY(DATE, 요일) : 특정 날짜 이후에 가장 가까운 해당 요일의 날짜를 반환
--> 결과의 값도 DATE 타입
SELECT SYSDATE, NEXT_DAY(SYSDATE, '금요일') FROM DUAL;
SELECT SYSDATE, NEXT_DAY(SYSDATE, '금') FROM DUAL;
SELECT SYSDATE, NEXT_DAY(SYSDATE, 5) FROM DUAL; -- 1: 일요일 ~ 7 : 토요일
SELECT SYSDATE, NEXT_DAY('98/01/09', 6) FROM DUAL;
SELECT SYSDSATE NEXT_DAY(SYSDATE, 'FRIDAY') FROM DUAL; -- 현재 언어가 KOREAN이라 안됨
...
SELECT TO_CHAR(1234, '99999') FROM DUAL; -- 5칸 공간 확보, 오른쪽 정렬, 빈칸 공백
...
-- 날짜 타입 => 문자 타입
SELECT SYSDATE FROM DUAL;
SELECT TO_CHAR(SYSDATE) FROM DUAL; -- 출력 값 클릭해보면 다름
...
SELECT TO_CHAR(HIRE_DATE, 'YYYY"년" MM"월" DD"일"') -- 없는 포맷 제시할때는 "" 로 묶기
FROM EMPLOYEE;
...
SELECT TO_DATE('970630', 'YYMMDD') FROM DUAL; -- 2097년 => 무조건 현재세기로 반영 무조건 20을 붙임
--SELECT EXTRACT(YEAR FROM TO_DATE('970630', 'YYMMDD')) FROM DUAL;

SELECT TO_DATE('140630', 'RRMMDD') FROM DUAL; -- 2014년
SELECT TO_DATE('970630', 'RRMMDD') FROM DUAL; -- 1997년 
...
SELECT TO_NUMBER('05123475') FROM DUAL; -- 5123475 => 0빠져서 숫자타입으로 저장됨

SELECT '10000000' + '55000' FROM DUAL; --> 오라클에서는 자동형변환 잘 돼있음!

SELECT '10,000,000' + '55,000' FROM DUAL; -- 오류남! 안에 숫자만 있어야 자동형변환된다
...
-- 전체 사원의 이름 보너스 포함 연봉
SELECT EMP_NAME,( SALARY * BONUS + SALARY )*12, ( SALARY * NVL(BONUS, 0) + SALARY )*12
FROM EMPLOYEE;
...
-- NVL2(컬럼, 반환값1, 반환값2)
-- 컬럼값이 존재할경우 반환값1 반환, NULL일 겨우 반환값2 반환
...
SELECT SUM(SALARY), /*EMP_NAME*/ -- 그룹함수 출력이될때에 한행씩 출력되는 단일행도 출력하려고하면 오류 발생
...
SELECT COUNT(DISTINCT DEPT_CODE) -- NULL 제외 카운트
FROM EMPLOYEE;

...
SELECT PROFESSOR_NAME AS "교수이름", EXTRACT(YEAR FROM SYSDATE)-EXTRACT(YEAR FROM TO_DATE('19'||SUBSTR(PROFESSOR_SSN,1,6),'YYYYMMDD')) AS "나이"
FROM TB_PROFESSOR
...
SELECT CLASS_NAME FROM TB_CLASS WHERE DEPARTMENT_NO IN (SELECT DEPARTMENT_NO FROM TB_DEPARTMENT WHERE CATEGORY = '예체능');
...
SELECT STUDENT_NO, STUDENT_NAME
FROM(SELECT STUDENT_NAME, STUDENT_NO, RANK() OVER(ORDER BY AVG(POINT) DESC) AS "순위"
    FROM TB_STUDENT
    JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
    JOIN TB_GRADE USING (STUDENT_NO)
    WHERE DEPARTMENT_NAME = '국어국문학과'
...
--"젂공평점"으로 표시되도록 하고, 평점은 소수점 핚 자리까지맊 반올림하여 표시되도록핚다.
...
AS SELECT DEPARTMENT_NAME, COUNT(*) AS "학생수"
    FROM TB_STUDENT
    JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
    GROUP BY DEPARTMENT_NAME;
...
SELECT CLASS_NO AS "과목번호", CLASS_NAME AS "과목이름", 누적수강생
...
-- 3.
SELECT STUDENT_NO, STUDENT_NAME, EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN,1,6))), PROFESSOR_NAME
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
JOIN TB_PROFESSOR ON (COACH_PROFESSOR_NO = PROFESSOR_NO)
WHERE DEPARTMENT_NAME = '국어국문학과';
...
AS SELECT STUDENT_NO, STUDENT_NAME, EXTRACT(YEAR FROM TO_DATE(SUBSTR(STUDENT_SSN,1,6)))AS "출생년도", PROFESSOR_NAME
    FROM TB_STUDENT
    JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
    JOIN TB_PROFESSOR ON (COACH_PROFESSOR_NO = PROFESSOR_NO)
    WHERE DEPARTMENT_NAME = '국어국문학과';

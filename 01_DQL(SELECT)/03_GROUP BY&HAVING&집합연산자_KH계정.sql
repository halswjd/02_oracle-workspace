...
-- 각 직급별 총 사원수, 보너스를 받는 사원수, 급여합, 평균급여, 최저급여, 최대급여 + 직급명 오름차순 정렬
SELECT JOB_CODE, COUNT(*) AS "총 사원수" , COUNT(BONUS) AS "보너스 받는 사원 수" ,
...
SELECT DEPT_CODE, COUNT(*) AS "총 사원수" , COUNT(BONUS) AS "보너스 받는 사원 수" ,
       SUM(SALARY) AS "급여합", ROUND(AVG(SALARY)) AS "평균급여" , MIN(SALARY) AS "최저급여", MAX(SALARY) AS "최대급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY 1;
...
SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
WHERE AVG(SALARY) >= 3000000 -- 오류 발생 (그룹함수 가지고 조건 제시시 WHERE절에서 안됨)
GROUP BY DEPT_CODE;
...
-- 부서별 보너스를 받는 사원이 없는 부서별 조회 부서코드, 보너스를 받는 사원의 수
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION -- 오류발생 : 각 쿼리문의 SELECT절에 작성되어있는 컬럼 개수 동일해야됨!!
...
FROM EMPLOYEE
WHERE SALARY > 3000000
ORDER BY EMP_NAME; -- ORDER BY절은 마지막에 한번만

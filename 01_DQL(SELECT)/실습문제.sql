...
SELECT EMP_NAME, SALARY*12 AS "연봉", (SALARY *BONUS+SALARY)*12 AS "총수령액", (SALARY *BONUS+SALARY)*12 - (SALARY*12*0.03) AS "실수령액"
FROM EMPLOYEE;
...
SELECT EMP_NAME, SALARY, ((SALARY *BONUS+SALARY)*12 - (SALARY*12*0.03)) AS "실수령액", HIRE_DATE
FROM EMPLOYEE
...
AND HIRE_DATE >= '900101' AND HIRE_DATE <= '001201' AND SALARY >= 2700000;

--15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
SELECT EMP_NAME, EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,6))) AS "생년", EXTRACT(MONTH FROM TO_DATE(SUBSTR(EMP_NO,1,6))) AS "생월", EXTRACT(DAY FROM TO_DATE(SUBSTR(EMP_NO,1,6))) AS "생일"
FROM EMPLOYEE;
...
SELECT *
FROM EMPLOYEE
--WHERE ENT_YN = 'N' AND 2023 - EXTRACT(YEAR FROM HIRE_DATE) >=20;

...
SELECT EMP_ID, EMP_NAME, 2023 - EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,6))) AS "나이", DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
...
FROM(SELECT EMP_ID, EMP_NAME, 2023 - EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,6))) AS "나이", DEPT_TITLE, JOB_NAME
        FROM EMPLOYEE
        JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
        JOIN JOB USING (JOB_CODE)
    ORDER BY 3)

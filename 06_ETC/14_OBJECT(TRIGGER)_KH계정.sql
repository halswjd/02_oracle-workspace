...
/*
    ...
    * 트리거 종류
    ...
    > ROW TRIGGER(행 트리거) : 해당 SQL문 실행할 때 마다 매번 트리거 실행
                ...
                > : NEW - BEFORE INSERT (추가된 자료), AFTER UPDATE(수정후자료)
...
CREATE TABLE TB_PRODETAIL(
    ...
    STATUS CHAR(6) CHECK(STATUS IN ('입고', '출고')) -- 입고/출고 상태
);
...
VALUES(SEQ_DCODE.NEXTVAL, 210, SYSDATE, 5, '출고'); -- PCODE컬럼값 FOREIGN KEY제약조건 참조하여 값 입력
...
/*
    ...
    UPDATE TB_PRODUCT
    SET STOCK = STOCK + 현재입고된수량(INSERT된 자료의 AMOUNT)
    WHERE PCODE = 입고된 상품번호(INSERT된 자료의 PCODE값);
    
    -- 상품이 출고된 경우 => 해당 상품 찾아서 재고수량 감소 UPDATE
    UPDATE TB_PRODUCT
    SET STOCK = STOCK - 현재출고된수량(INSERT된 자료의 AMOUNT)

...
/*
    ...
    1_1) 일반타입 변수 선언 및 초기화
         [표현식] 변수명 [CONSTANT -> 상수처리] 자료형 [:= 값];
*/

DECLARE
    EID NUMBER; -- 선언
    ENAME VARCHAR2(20); -- 선언
...
/*
    ...
    각 자료형이 EMPLOYEE 혹은 DEPARTMENT 테이블을 참조하도록
...
DECLARE
    E EMPLOYEE%ROWTYPE;
BEGIN
    SELECT * -- ROW타입을 쓸 때에는 모든 컬럼에 해당하는 값을 넣어야함
    INTO E
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;
...
-- 1) IF 조건식 THEN 실행내용 END IF; (자바에서 단독IF문)
...
-- 2) IF 조건식 THEN 실행내용 ELSE 실행내용 END IF; (자바에서 IF-ELSE문)
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    SALARY EMPLOYEE.SALARY%TYPE;
...
------------------------------------------------------------------------------------------
-- 3) IF 조건식1 THEN 실행내용1 ELSIF 조건식2 THEN 실행내용2 ELSE 실행내용 END IF; (자바에서 IF - ELSE IF - ELSE 문)
...
-- 4) CASE 비교대상자 WHEN 동등비교할 값1 THEN 결과값1 WHEN 비교값1 THEN 결과값2... ELSE 결과값 END;
...
-- 보너스가 있으면 보너스 포함 연봉
-- 출력예시
-- 급여 이름 \999,999,9999 (8000000 선동일 \124,800,800)
...
DECLARE
    ...
    BSAL NUMBER;
BEGIN
    SELECT EMP_NAME, SALARY, NVL(BONUS,0) -- (SALARY * NVL(BONUS,0) + SALARY)*12
    INTO ENAME, SAL, BONUS
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;

    IF BONUS = 0 THEN BSAL := SAL *12;
    ELSE BSAL := (SAL * BONUS + SAL)*12;
    END IF;
...
/*
    ...
    [표현식]
    FOR 변수 [REVERSE -> 값을 감소] IN 초기값.. 최종값
    LOOP
    
    END LOOP;
    
    -> DECLARE문 필요 없다는 장점이 있음
*/
...
BEGIN
    ...
    INTO EID, ENAME
    FROM EMPLOYEE
    WHERE MANAGER_ID = &사수사번; -- 211 장쯔위 200 TOO MANY 202 전혀없음

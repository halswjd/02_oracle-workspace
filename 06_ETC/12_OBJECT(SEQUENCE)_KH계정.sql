...
/*
    ...
    [상세 표현식]
    CREATE SEQUENCE 시퀀스명
    [START WITH 시작숫자]           -- 처음 발생시킬 시작값 지정 (생략시 기본값은 1)
    [INCREAMENT BY 숫자]           -- 몇 씩 증가시킬껀지 (기본값 1)
    [MAXVALUE 숫자]                -- 최대값 지정 (기본값 겁나큼...) --> 앵간해선 설정X
    [MINVALUE 숫자]                -- 최소값 지정 (기본값 1)
    [CYCLE | NOCYCLE]             -- 값 순환 여부 지정 (기본값 NOCYCLE) => 최대값 찍고 처음부터 다시 돌아와서 시작할 수 있게함
    [NOCACHE|CACHE 바이트크기]      -- 캐시메모리 할당 (기본값 CACHE 20) --> NOCACHE로 하는 습관
    
    * 캐시메모리 : 임시공간, 미리 발생될 값들을 생성해서 저장해두는 공간
...
-- [참고] 현재 계정이 소유하고 있는 시퀀스들의 구조를 보고자 할 때 쿼리문
...
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 310 + 1 => 320

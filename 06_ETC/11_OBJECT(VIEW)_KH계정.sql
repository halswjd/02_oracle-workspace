...
-- 관리자 계정에 접속해서 권한을 부여해줘야 한다. (GRANT할때만 관리자 계정으로 바꿔줘야 함.)
GRANT CREATE VIEW TO KH;

-- 이건 실제 있는 테이블은 아님. 그래서 가상, 논리 테이블 인거임
SELECT *
FROM VW_EMPLOYEE;
...
-- OR PLACE를 달지 않을 경우는 이미 해당 이름을 쓰는 뷰가 있다고 에러가 난다.
...
-- 전 사원의 사번, 이름, 직급명, 성별(남/여), 근무년수를 조회할 수 있는 SELECT문을 뷰(VIEW_EMP_JOB)로 정의
CREATE OR REPLACE VIEW VW_EMP_JOB
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여') AS "성별(남/여)",
...
-- 아래와 같은 방식으로도 별칭 부여 가능 (단, 모든 컬럼에 대해서 별칭 부여 필요)
...
-- 근데 잘 안되는 경우가 많아서 실제로 많이 쓰지는 않음 (뷰는 조회가 목적이라)

CREATE OR REPLACE VIEW VW_JOB
AS SELECT JOB_CODE, JOB_NAME
     FROM JOB;
     
SELECT * FROM VW_JOB; -- 논리적인 테이블 (실제데이터가 담겨있는건 아님)
SELECT * FROM JOB; -- 베이스 테이블 (씰제 데이터가 담겨있는 테이블)

-- 뷰를 통해서 INSERT
INSERT INTO VW_JOB VALUES ('J8', '인턴');
-- 가상테이블이지만 뷰에 DML문 작업을 하면 실제테이블에 반영됨
...
-----------------------------------------------------------------------------------
/*
    * 단, DML 명령어로 조작이 불가능한 경우 더 많음!!
    1) 뷰에 정의되어 있지 않은 컬럼을 조작하려고 하는 경우
    2) 뷰에 정의되어 있지 않은 컬럼중에 베이스테이블 상에 NOT NULL 제약조건이 지정되어 있는 경우 => 어떤건 되고 어떤건 안됨
...
-- INSERT => 에러
INSERT INTO VW_JOB VALUES('인턴'); -- 실제 베이스 테이블에 INSERT시  (NULL, '인턴')추가 -> NOT NULL 제약조건 위배
...
DELETE FROM VW_JOB
WHERE JOB_NAME = '사원';
-- ORA-02292: integrity constraint (KH.SYS_C007125) violated - child record found
...
-- EMPLOYEE 테이블에는 연봉이라는 컬럼이 없음!

-- UPDATE
-- 사번이 200번인 사원의 연봉을 8000만원으로 변경
UPDATE VW_EMP_SAL
SET 연봉 = 80000000
WHERE EMP_ID = 200; --> 에러
-- ORA-01733: virtual column not allowed here
...
UPDATE VW_EMP_SAL
SET SALARY = 7000000
WHERE EMP_ID = 200; -- 성공 (베이스테이블 또한 다 변경됨)
...
-- INSERT => 에러
INSERT INTO VW_GROUPDEPT VALUES('D3', 80000000, 4000000);
-- ORA-01733: virtual column not allowed here
-- 실제 EMPLOYEE 테이블에 데이터 넣기가 애매함.. 그래서 안됨
...
UPDATE VW_JOINEMP
SET EMP_NAME = '선동이'
WHERE EMP_ID = 200; -- 문제없이 됨, 베이스테이블에 반영
...
SET DETP_TITLE = '선동이'
WHERE EMP_ID = 200; -- 에러
-- EMPLOYEE 테이블에는 DEPT_TITLE 컬럼이 존재하지 않아서
...
/*
    * VIEW 옵션
    ...
    2) FORCE | NOFORCE
      ...
      > NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이여야만 뷰가 생성되게 하는 구문 (생략시 기본값)
    3) WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML 가능하도록 하는 옵션
    4) WITH READ ONLY : 뷰에 대해 조회만 가능하게 하는 옵션 (DML문 수행 불가)
...
UPDATE VW_EMP
 SET SALARY = 2000000
 WHERE EMP_ID = 200; --> 잘 변경됨!, 200번 사원 서브쿼리 조건에 부합하지 않아 제외됨
 
ROLLBACK;

--    WITH CHECK OPTION 줘서 뷰 생성
CREATE OR REPLACE VIEW VW_EMP
AS SELECT *
    FROM EMPLOYEE
   WHERE SALARY >= 3000000
...
UPDATE VW_EMP
 SET SALARY = 2000000
 WHERE EMP_ID = 200; --> 오류발생 : 서브쿼리에 기술한 조건에 부합되지 않기 때문에 변경 불가
-- ORA-01402: view WITH CHECK OPTION where-clause violation

UPDATE VW_EMP
 SET SALARY = 4000000
 WHERE EMP_ID = 200; -->  서브쿼리에 기술한 조건에 부합돼서 변경 가능!

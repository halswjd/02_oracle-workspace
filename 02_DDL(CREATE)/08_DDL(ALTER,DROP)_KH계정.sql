...
/*
    ...
    * 변경할 내용
    1) 컬럼 추가/수정/삭제
    2) 제약조건 추가/삭제 , 수정은 불가 (수정하고자 한다면 삭제하고 새로이 추가)
    3) 컬럼명/제약조건명/테이블명 수정
...
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER; -- 오류발생 : 이미 데이터에 숫자 아닌것도 들어있어서
-- 존재하는 데이터가 없어야만 자료형 바꿀 수 있음
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(10); -- 오류발생 : 이미 데이터에 10바이트보다 큰 데이터가 담겨있기 때문

-- DEPT_COPY
-- 문제1) DEPT_TITLE 컬럼을 VARCHAR2(50)로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(50);

-- 문제2) LOCATION_ID 컬럼을 VARCHAR2(4)로 변경
ALTER TABLE DEPT_COPY MODIFY LOCATION_ID VARCHAR2(4);

-- 문제3) LNAME 컬럼의 기본값을 '미국'으로 변경
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT '미국'; -- 이미 담겨있던 데이터의 DEFAULT값은 바뀌지 않음
...
-- 디폴트값을 바꾼다고 해서 이전에 추가된 데이터가 바뀌는 것은 아님!
...
/*
    ...
    NOT NULL : MODIFY 컬럼명 NOT NULL | NULL(허용) **
    
    제약조건명 지정하고자 한다면 [CONSTRAINT 제약조건명] 제약조건 
*/

-- DEPT_COPY 테이블의 DEPT_ID에 PRIMARY KEY 제약조건 추가 ADD
...
-- SYS_C007137 LOCATION_ID NN 자기꺼 가져오기
...
-- 단, 어디서 참조되고 있는 부모테이블은 함부로 삭제 안됨!!
-- 만약 삭제하고자 한다면
